...
namespace CCat {
	// TODO mark text
	// TODO skip word with STRG + Arrow
	// TODO mark on word thurther with STRG + SHIFT + Arrow
	// TODO select CursorPos with Mouse
	...
	private:
		...
		void keyEvent(sf::Event event) {
			...
			if (selected && event.type == sf::Event::KeyPressed) {
				...
				case sf::Keyboard::Up:
					// TODO
					if (mouseRow > 0) {
						int rowCharCount0 = charsPerRow.at(mouseRow);
						mouseRow++;
		...
		void calcCharsPerCol() {
			int r = 0;
			for (auto& c : str) {
				if (c != '\n')
					r++;
				else {
					charsPerRow.emplace_back(r);
					r = 0;
				}
			}
		}
